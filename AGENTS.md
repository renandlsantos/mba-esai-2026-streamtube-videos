...
- **Message Queue** (BullMQ/Redis, fase 03) → video processing job queue
...
## Codex — portabilidade do workflow

Fonte portada de CLAUDE.md; regras em .claude/rules continuam referência obrigatória. Skills equivalentes estão em .agents/skills/speckit-*. O pipeline original é conduzido manualmente em sequência: research→context→validate→resolve→build→implement. O formato de docs/phases/phase-03-videos é preservado. As leituras dos sub-agents do Claude são realizadas pelo agente Codex sem invocar sub-agentes extras. Context7 foi consultado pelo servidor MCP Docker; configuração de exemplo em .codex/config.toml.example e evidência em docs/phases/phase-03-videos/library-refs.md. Consulte fontes oficiais e registre as limitações de versão.

Não executar comandos de Node/npm no host. Infra isolada mba-esai-296. Nenhum conteúdo privado das aulas pode ser publicado.
