...
function makeDataSource(manager: ReturnType<typeof makeManager>): DataSource {
  const source = new DataSource({ type: 'postgres' });
  jest
    .spyOn(source, 'transaction')
    .mockImplementation((isolationOrCallback, callback) => {
      const action =
        typeof isolationOrCallback === 'function'
          ? isolationOrCallback
          : callback;
      if (!action) throw new Error('Missing transaction callback');
      const entityManager = source.manager;
      jest.spyOn(entityManager, 'findOne').mockImplementation(manager.findOne);
      jest.spyOn(entityManager, 'create').mockImplementation(manager.create);
      jest.spyOn(entityManager, 'save').mockImplementation(manager.save);
